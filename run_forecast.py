...
def fetch_market_data():
    ...
    try:
        egp = yf.download("EGP=X", period="2d", interval="1d", progress=False)
        if isinstance(egp.columns, pd.MultiIndex):
            egp.columns = [col[0] for col in egp.columns]
        dollar_rate = float(egp["Close"].dropna().iloc[-1])
        log(f"  Dollar rate: {dollar_rate:.2f} EGP/USD (Yahoo Finance)")
    except Exception as e:
        log(f"  Yahoo failed: {e}, trying open.er-api...")
        try:
            r = requests.get("https://open.er-api.com/v6/latest/USD", timeout=10)
            dollar_rate = float(r.json()["rates"]["EGP"])
            log(f"  Dollar rate: {dollar_rate:.2f} EGP/USD (open.er-api fallback)")
        except:
            dollar_rate = 49.10
