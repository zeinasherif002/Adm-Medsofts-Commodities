...
export default function App() {
  ...
  var [chatMessages,setChatMessages]=useState([{role:"assistant",content:"Hello! I'm your AI market analyst. Ask me anything about the "+commodity+" market — price movements, WASDE reports, weather impacts, or trading recommendations."}]);
  ...
  function sendChatMessage(){
    ...
    setChatInput("");
    setChatMessages(function(prev){return [...prev,{role:"user",content:userMsg}];});
    setChatLoading(true);

    var context = "You are an expert commodity market analyst for ADM MedSofts Egypt trading team. "+
      "Current data - Commodity: "+commodity.toUpperCase()+
      (L ? ", CBOT: "+L.closing_cbot+" c/bu, Date: "+L.date+", Dollar: "+L.dollar_rate+" EGP/USD" +
      (isWheat ? ", 11.5%: "+Math.round(L.arg_price)+" EGP, 12.5%: "+Math.round(L.brz_price)+" EGP" :
      ", ARG: "+Math.round(L.arg_price)+" EGP, BRZ: "+Math.round(L.brz_price)+" EGP") : "")+
      (cropCondition ? ", USDA G+E: "+(cropCondition.excellent+cropCondition.good)+"%" : "")+
      (wasde ? ", Pre-WASDE: "+wasde.price_impact : "")+
      ". Use web search to find latest news about price movements. Answer in English, be specific and actionable.";

    var msgs = chatMessages.filter(function(m){return m.role!=="assistant"||chatMessages.indexOf(m)>0;})
    ...
    fetch("https://cupcsspfmkgbcovtgszm.supabase.co/functions/v1/claude-chat",{
      ...
      body:JSON.stringify({messages:msgs,system:context})
    })
    .then(function(r){return r.json();})
    .then(function(data){
      var text = data.text || data.error || "No response";
      setChatMessages(function(prev){return [...prev,{role:"assistant",content:text}];});
      setChatLoading(false);
      setTimeout(function(){var el=document.getElementById("chat-messages");if(el)el.scrollTop=el.scrollHeight;},100);
    })
    .catch(function(e){
  ...
  function fetchGlobalData(){
    ...
    var prompt = comm==="wheat" ?
      "What are the current world wheat supply and demand figures for 2025/26 marketing year? Search for USDA latest estimates for: world wheat ending stocks in MMT, world wheat total use in MMT, Russia wheat production in MMT, Ukraine wheat production in MMT, EU wheat production in MMT. Use the most recent available data from USDA, IGC, or any reliable source. Return ONLY a raw JSON object with numeric values, no text, no backticks, no explanation: world_ending_stocks, world_consumption, russia_production, ukraine_production, eu_production." :
      "What are the current world corn supply and demand figures for 2025/26 marketing year? Search for USDA latest estimates for: world corn ending stocks in MMT, world corn total use in MMT, Brazil corn production in MMT, Argentina corn production in MMT, Ukraine corn production in MMT. Use the most recent available data from USDA, IGC, or any reliable source. Return ONLY a raw JSON object with numeric values, no text, no backticks, no explanation: world_ending_stocks, world_consumption, brazil_production, argentina_production, ukraine_production.";
    fetch("https://cupcsspfmkgbcovtgszm.supabase.co/functions/v1/claude-chat",{
      method:"POST",
      headers:{"Content-Type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN1cGNzc3BmbWtnYmNvdnRnc3ptIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyNzI4ODMsImV4cCI6MjA5NDg0ODg4M30.Y8o09mcvdJuSSfgsVGnhoUyRpIUPVl8-gkigJXXee8E"},
  ...
  function sendWasdeChat(){
    var userMsg = wasdeChatInput.trim();
    setWasdeChatInput("");
    ...
    .then(function(r){return r.json();})
    .then(function(data){
      setWasdeChat(function(prev){return [...prev,{role:"assistant",content:data.text||"No response"}];});
      setWasdeChatLoading(false);
      setTimeout(function(){var el=document.getElementById("wasde-chat-messages");if(el)el.scrollTop=el.scrollHeight;},100);
    })
    .catch(function(e){
      setWasdeChat(function(prev){return [...prev,{role:"assistant",content:"Error: "+e.message}];});
      setWasdeChatLoading(false);
    });
  }
    ...
    fetch(SUPABASE_URL+"/rest/v1/ai_analysis?commodity=eq."+commodity+"&order=created_at.desc&limit=1",{headers:HEADERS})
      ...
      .then(function(data){
        if(data&&data.length>0&&data[0].analysis){
          ...
          setAi("No AI analysis available yet. Analysis is generated automatically each day when the market closes.");
        }
        setAiLoad(false);
      })
      .catch(function(e){
        setAi("Error loading analysis: "+e.message);
        setAiLoad(false);
      });
  }
  ...
  var nav=[["overview","Overview"],["charts","Charts"],["analysis","AI Analysis"],["returns","Returns"],["table","Raw Data"],["weekly","Weekly Forecast"],["wasde","WASDE Report"],["news","Market News"]];

  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter','DM Sans','Segoe UI',sans-serif",display:"flex"}}>
      <div style={{width:230,minHeight:"100vh",background:C.sb,display:"flex",flexDirection:"column",position:"fixed",top:0,left:isMobile?(mobileMenuOpen?0:-230):0,bottom:0,overflowY:"auto",borderRight:"1px solid "+C.border,zIndex:1000,transition:"left 0.25s ease"}}>
        <div style={{padding:"22px 20px 18px",borderBottom:"1px solid "+C.border}}><div style={{display:"flex",alignItems:"center",gap:12}}><div><img src="/Adm-Medsofts-Commodities/media.jpg" alt="ADM MedSofts" style={{height:32,objectFit:"contain",filter:"none"}}/><div style={{fontSize:10,color:C.sub,marginTop:2}}>Commodity Intelligence</div></div></div></div>
      ...
      {isMobile&&mobileMenuOpen&&<div onClick={function(){setMobileMenuOpen(false);}} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:999}}/>}
      {isMobile&&<button onClick={function(){setMobileMenuOpen(!mobileMenuOpen);}} style={{position:"fixed",top:14,left:14,zIndex:1001,width:40,height:40,borderRadius:10,border:"1px solid "+C.border,background:C.card,color:C.text,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>☰</button>}
      <div style={{marginLeft:isMobile?0:230,flex:1,padding:isMobile?"60px 16px 16px":"28px 32px",minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}><div><h1 style={{fontSize:24,fontWeight:900,margin:0,color:C.text,letterSpacing:"-0.5px"}}>{cc?cc.label:""} Market</h1><div style={{fontSize:12,color:C.sub,marginTop:4}}>CBOT Futures · EGP Local Prices · {L?L.date:"—"} · <span style={{color:C.amber,fontWeight:600}}>{isWheat?"ZW=F (Wheat Front Month)":"ZCU26 (Sep 2026)"}</span></div></div><div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        {loading?(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:300,color:C.sub}}>Loading...</div>):!L?(<div style={{textAlign:"center",padding:80,color:C.sub}}>No data</div>):(
          <div>
            {(tab==="overview"||tab==="charts"||tab==="analysis")&&(<div style={{marginBottom:20}}><div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:isMobile?8:14,marginBottom:14}}><GCard label="CBOT Close" value={fmt(L.closing_cbot)} unit="cents / bushel" gradient={"linear-gradient(135deg,"+C.blue+","+C.purple+")"} delta={cbotD} pct={cbotPct}/><GCard label={label1} value={fmtK(L.arg_price)} unit="Egyptian Pound" gradient={"linear-gradient(135deg,"+C.green+",#059669)"} delta={argD}/><GCard label={label2} value={fmtK(L.brz_price)} unit="Egyptian Pound" gradient={"linear-gradient(135deg,"+C.amber+",#d97706)"}/><GCard label="Dollar Rate" value={fmt(L.dollar_rate)} unit="EGP / USD" gradient={"linear-gradient(135deg,"+C.pink+","+C.purple+")"}/>
            </div><div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(7,1fr)",gap:isMobile?8:12,marginBottom:14}}>{[
            ...
            {tab==="analysis"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>{[{label:"Overall",value:sig,color:sigC,sub:buys+"B/"+sells+"S"},{label:"RSI Signal",value:rsiSig,color:rsiSig==="BUY"?C.green:rsiSig==="SELL"?C.red:C.amber,sub:"RSI="+(rsi?rsi.toFixed(1):"—")},{label:"MACD Signal",value:macdSig,color:macdSig==="BUY"?C.green:macdSig==="SELL"?C.red:C.amber,sub:macd&&macd.histogram?(macd.histogram>0?"Mom+":"Mom-"):"—"},{label:"Trend",value:trendSig,color:trendSig==="BUY"?C.green:trendSig==="SELL"?C.red:C.amber,sub:trend}].map(function(c,i){return(<div key={i} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:"18px 20px",textAlign:"center"}}><div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:12}}>{c.label}</div><div style={{fontSize:18,fontWeight:800,color:c.color,border:"1px solid "+c.color+"50",display:"inline-block",padding:"4px 16px",borderRadius:8,marginBottom:8}}>{c.value}</div><div style={{fontSize:11,color:C.sub}}>{c.sub}</div></div>);})}</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>{[{label:"Z-Score",value:zs.toFixed(2),color:Math.abs(zs)>2?C.red:C.cyan,sub:Math.abs(zs)>2?"Extreme":"Normal"},{label:"Support",value:fmt(sr.support),color:C.green,sub:"floor"},{label:"Resistance",value:fmt(sr.resistance),color:C.red,sub:"ceiling"},{label:"Bollinger",value:boll&&boll.upper?(L.closing_cbot>boll.upper?"Above":L.closing_cbot<boll.lower?"Below":"Inside"):"—",color:C.purple,sub:boll&&boll.upper?"U:"+fmt(boll.upper):""}].map(function(c,i){return(<div key={i} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:"16px 18px"}}><div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>{c.label}</div><div style={{fontSize:20,fontWeight:800,color:c.color}}>{c.value}</div><div style={{fontSize:11,color:C.sub}}>{c.sub}</div></div>);})}</div><div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,overflow:"hidden"}}><div style={{padding:"16px 20px",borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:14,fontWeight:700,color:C.text}}>🤖 AI Market Assistant </div><div style={{fontSize:11,color:C.sub}}>Powered by Claude · Ask anything about the market</div></div><div style={{height:320,overflowY:"auto",padding:"16px 20px",display:"flex",flexDirection:"column",gap:12}} id="chat-messages">{chatMessages.map(function(msg,i){return(<div key={i} style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"80%",padding:"10px 14px",borderRadius:12,background:msg.role==="user"?"linear-gradient(135deg,"+C.blue+","+C.purple+")":C.bg,border:msg.role==="user"?"none":"1px solid "+C.border,color:msg.role==="user"?"#fff":C.text,fontSize:12,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{msg.content}</div></div>);})}{chatLoading&&<div style={{display:"flex",justifyContent:"flex-start"}}><div style={{padding:"10px 14px",borderRadius:12,background:C.bg,border:"1px solid "+C.border,color:C.sub,fontSize:12}}>Analyzing market data...</div></div>}</div><div style={{padding:"12px 16px",borderTop:"1px solid "+C.border,display:"flex",gap:8}}><input value={chatInput} onChange={function(e){setChatInput(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChatMessage();}}} placeholder={"Ask about "+commodity+" market... (e.g. why did prices drop today?)"} style={{flex:1,background:C.bg,border:"1px solid "+C.border,borderRadius:8,padding:"8px 12px",color:C.text,fontSize:12,outline:"none"}}/><button onClick={sendChatMessage} disabled={chatLoading||!chatInput.trim()} style={{padding:"8px 16px",borderRadius:8,border:"none",background:"linear-gradient(135deg,"+C.blue+","+C.purple+")",color:"#fff",fontSize:12,cursor:"pointer",fontWeight:600,opacity:chatLoading||!chatInput.trim()?0.5:1}}>Send</button></div></div></div>)}
            ...
            {tab==="wasde"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
  {commodity==="wheat"&&<div style={{background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.4)",borderRadius:16,padding:"16px 20px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{fontSize:13,fontWeight:700,color:C.amber}}>⚡ USDA Quarterly Acreage & Grain Stocks Report — June 30, 2026</div>
      <div style={{fontSize:11,padding:"3px 10px",borderRadius:6,fontWeight:700,background:"rgba(251,191,36,0.15)",color:C.amber}}>Released 12:00 PM ET / 7:00 PM Cairo</div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:10}}>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Planted Acres (actual)</div><div style={{fontSize:14,fontWeight:700,color:C.green}}>42.7M</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>vs Mar est. 43.8M ↓ Bullish</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Jun 1 Grain Stocks</div><div style={{fontSize:14,fontWeight:700,color:C.text}}>920M bu</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>Jun 1 wheat stocks</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Price Reaction</div><div style={{fontSize:14,fontWeight:700,color:C.green}}>Supportive</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>fewer acres than expected</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Market Bias</div><div style={{fontSize:14,fontWeight:700,color:C.green}}>BULLISH</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>ahead of Jul 10 WASDE</div></div>
    </div>
    <div style={{fontSize:11,color:C.sub,fontStyle:"italic"}}>Wheat planted acres fell to 42.7M vs March estimate of 43.8M — a bullish surprise of ~1M fewer acres. June 1 stocks at 920M bu. Smaller crop = price support heading into July 10 WASDE first yield estimate.</div>
  </div>}
{commodity==="corn"&&<div style={{background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.4)",borderRadius:16,padding:"16px 20px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{fontSize:13,fontWeight:700,color:C.amber}}>⚡ USDA Quarterly Acreage & Grain Stocks Report — June 30, 2026</div>
      <div style={{fontSize:11,padding:"3px 10px",borderRadius:6,fontWeight:700,background:"rgba(251,191,36,0.15)",color:C.amber}}>Released 12:00 PM ET / 7:00 PM Cairo</div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:10}}>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Planted Acres (actual)</div><div style={{fontSize:14,fontWeight:700,color:C.green}}>95.3M</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>vs est. 95.3M — in line</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Jun 1 Grain Stocks</div><div style={{fontSize:14,fontWeight:700,color:C.text}}>2.96B bu</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>Jun 1 corn stocks</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Price Reaction</div><div style={{fontSize:14,fontWeight:700,color:C.amber}}>Minimal</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>acres in line w/ est.</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Market Bias</div><div style={{fontSize:14,fontWeight:700,color:C.amber}}>NEUTRAL</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>ahead of Jul 10 WASDE</div></div>
    </div>
    <div style={{fontSize:11,color:C.sub,fontStyle:"italic"}}>Corn planted acres came in at 95.3M — exactly in line with pre-report estimates. June 1 stocks at 2.96B bu suggest adequate supply. No major surprises = NEUTRAL market reaction. Watch July 10 WASDE for first official 2026 yield estimate, which will be the next major price catalyst.</div>
  </div>}

  {wasde&&commodity==="corn"&&<div style={{background:C.card,border:"1px solid rgba(27,58,140,0.3)",borderRadius:16,padding:"16px 20px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:13,fontWeight:700,color:C.blue}}>📊 Corn Pre-WASDE Analysis — Next Report: {wasde.report_date}</div><div style={{fontSize:11,padding:"3px 10px",borderRadius:6,fontWeight:700,background:wasde.price_impact&&wasde.price_impact.includes("BULL")?"rgba(92,184,92,0.15)":wasde.price_impact&&wasde.price_impact.includes("BEAR")?"rgba(248,113,113,0.15)":"rgba(251,191,36,0.15)",color:wasde.price_impact&&wasde.price_impact.includes("BULL")?C.green:wasde.price_impact&&wasde.price_impact.includes("BEAR")?C.red:C.amber}}>{wasde.price_impact&&wasde.price_impact.split(" - ")[0]}</div></div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>{[{label:"Planted Acres",value:wasde.planted_acres?(wasde.planted_acres/1e6).toFixed(1)+"M":"—"},{label:"G+E Condition",value:wasde.ge_condition?wasde.ge_condition+"%":"—"},{label:"Est. Yield",value:wasde.estimated_yield?wasde.estimated_yield+" bu/ac":"—"},{label:"Est. Production",value:wasde.estimated_production?wasde.estimated_production+"B bu":"—"}].map(function(item,i){return(<div key={i} style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>{item.label}</div><div style={{fontSize:14,fontWeight:700,color:C.text}}>{item.value}</div></div>);})}</div><div style={{fontSize:12,color:C.text,lineHeight:1.6,marginBottom:10}}>{wasde.price_impact&&wasde.price_impact.split(" - ")[1]}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={{background:"rgba(92,184,92,0.08)",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.green,fontWeight:700,marginBottom:4}}>BULLISH SCENARIO</div><div style={{fontSize:12,color:C.text}}>Yield drops to {wasde.bullish_scenario_yield} bu/ac → prices rally</div></div><div style={{background:"rgba(248,113,113,0.08)",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.red,fontWeight:700,marginBottom:4}}>BEARISH SCENARIO</div><div style={{fontSize:12,color:C.text}}>Yield rises to {wasde.bearish_scenario_yield} bu/ac → prices fall</div></div></div></div>}
  ...
  {(!wasde&&!wasdeWheat)&&<div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:"32px",textAlign:"center",color:C.sub}}>No WASDE data available. Switch to Corn to see pre-WASDE analysis.</div>}<div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}><button onClick={generateWasdeReport} style={{padding:"9px 20px",borderRadius:10,border:"1px solid "+C.blue,background:"rgba(27,58,140,0.15)",color:C.blue,fontSize:12,cursor:"pointer",fontWeight:700}}>📄 Print / Export PDF</button></div>
...
{tab==="wasde"&&L&&sr&&(<div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:"18px 20px"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
    <div onClick={function(){setSelectedScenario("bullish");}} style={{background:selectedScenario==="bullish"?"rgba(92,184,92,0.2)":"rgba(92,184,92,0.08)",border:"1px solid rgba(92,184,92,0.3)",borderRadius:10,padding:"14px",cursor:"pointer",transition:"all 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:11,color:C.green,fontWeight:700}}>🐂 BULLISH</div><div style={{fontSize:13,fontWeight:800,color:C.green,background:"rgba(92,184,92,0.15)",padding:"2px 8px",borderRadius:6}}>{isWheat?"50%":cropCondition?(cropCondition.excellent+cropCondition.good)<60?"40%":(cropCondition.excellent+cropCondition.good)<68?"30%":(cropCondition.excellent+cropCondition.good)<75?"25%":"20%":"30%"}</div></div>
    ...
    <div onClick={function(){setSelectedScenario("neutral");}} style={{background:selectedScenario==="neutral"?"rgba(251,191,36,0.2)":"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.3)",borderRadius:10,padding:"14px",cursor:"pointer",transition:"all 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:11,color:C.amber,fontWeight:700}}>➡️ NEUTRAL</div><div style={{fontSize:13,fontWeight:800,color:C.amber,background:"rgba(251,191,36,0.15)",padding:"2px 8px",borderRadius:6}}>{isWheat?"30%":cropCondition?(cropCondition.excellent+cropCondition.good)<60?"30%":(cropCondition.excellent+cropCondition.good)<68?"40%":(cropCondition.excellent+cropCondition.good)<75?"40%":"35%":"35%"}</div></div>
    ...
    <div onClick={function(){setSelectedScenario("bearish");}} style={{background:selectedScenario==="bearish"?"rgba(248,113,113,0.2)":"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:10,padding:"14px",cursor:"pointer",transition:"all 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:11,color:C.red,fontWeight:700}}>🐻 BEARISH</div><div style={{fontSize:13,fontWeight:800,color:C.red,background:"rgba(248,113,113,0.15)",padding:"2px 8px",borderRadius:6}}>{isWheat?"20%":cropCondition?(cropCondition.excellent+cropCondition.good)<60?"30%":(cropCondition.excellent+cropCondition.good)<68?"30%":(cropCondition.excellent+cropCondition.good)<75?"35%":"45%":"35%"}</div></div>
  ...
  {selectedScenario&&<div style={{background:C.bg,border:"1px solid "+(selectedScenario==="bullish"?"rgba(92,184,92,0.4)":selectedScenario==="bearish"?"rgba(248,113,113,0.4)":"rgba(251,191,36,0.4)"),borderRadius:12,padding:"14px 16px"}}>
    <div style={{fontSize:12,fontWeight:700,color:selectedScenario==="bullish"?C.green:selectedScenario==="bearish"?C.red:C.amber,marginBottom:8}}>{selectedScenario==="bullish"?"Bullish Case:":selectedScenario==="bearish"?"Bearish Case:":"Neutral Case:"}</div>
    {selectedScenario==="bullish"&&isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Wheat planted acres fell to 42.7M vs March estimate of 43.8M</li><li>G+E crop condition at {wasdeWheat?wasdeWheat.ge_condition:48}% below historical average of 52%</li><li>Estimated production below last year — smaller supply = price support</li><li>Risk: Black Sea supply competition could limit upside</li></ul>}
    {selectedScenario==="bullish"&&!isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Corn G+E at {cropCondition?(cropCondition.excellent+cropCondition.good):68}% — any drop below 65% signals yield stress</li><li>Acres in line at 95.3M — no supply surprise from acreage report</li><li>Any USDA yield cut on July 10 would be price supportive</li><li>Risk: Good summer weather in July could limit rally</li></ul>}
    {selectedScenario==="neutral"&&isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Wheat acreage surprise (42.7M) already partially priced in by market</li><li>G+E below avg but harvest near completion — limited weather risk</li><li>USDA July yield estimate likely close to trend (49-50 bu/ac)</li><li>Global supply from Black Sea region offsetting US tightness</li></ul>}
    {selectedScenario==="neutral"&&!isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Corn planted acres exactly in line with estimates (95.3M)</li><li>G+E at {cropCondition?(cropCondition.excellent+cropCondition.good):68}% near historical average of 68%</li><li>No major supply or demand surprise expected before WASDE</li><li>July WASDE first yield estimate likely near trend (181-182 bu/ac)</li></ul>}
    {selectedScenario==="bearish"&&isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Black Sea exports remain competitive and abundant globally</li><li>If USDA raises yield estimate above 50 bu/ac on July 10</li><li>Demand weakness from key importers including Egypt and Middle East</li><li>Risk: Weather disruption could quickly reverse bearish view</li></ul>}
    {selectedScenario==="bearish"&&!isWheat&&<ul style={{fontSize:12,color:C.text,lineHeight:2,margin:0,paddingLeft:16}}><li>Corn G+E holding above 65% — adequate crop development so far</li><li>South American competition (Brazil record crop) weighing on exports</li><li>If USDA raises yield above 182 bu/ac on July 10 WASDE</li><li>Risk: Any weather stress in July could quickly shift bias bullish</li></ul>}
  </div>}
  <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:"18px 20px"}}>
    ...
    {globalData&&!globalData.error&&<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
      ...
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>STU Ratio</div><div style={{fontSize:15,fontWeight:700,color:globalData.world_ending_stocks&&globalData.world_consumption&&(globalData.world_ending_stocks/globalData.world_consumption)<0.25?C.red:C.green}}>{globalData.world_ending_stocks&&globalData.world_consumption?(globalData.world_ending_stocks/globalData.world_consumption*100).toFixed(1)+"%":"—"}</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>{isWheat?"Russia Production":"Brazil Production"}</div><div style={{fontSize:15,fontWeight:700,color:C.green}}>{isWheat?globalData.russia_production:globalData.brazil_production} MMT</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>{isWheat?"Ukraine Production":"Argentina Production"}</div><div style={{fontSize:15,fontWeight:700,color:C.green}}>{globalData.ukraine_production} MMT</div></div>
      <div style={{background:C.bg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:C.sub,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>{isWheat?"EU Production":"Ukraine Production"}</div><div style={{fontSize:15,fontWeight:700,color:C.amber}}>{isWheat?globalData.eu_production:globalData.ukraine_production} MMT</div></div>
    </div>}
  </div>
...
// cache bust Tue Jun 23 13:53:47 UTC 2026
