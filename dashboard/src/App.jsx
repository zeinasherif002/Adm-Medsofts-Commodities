...
{tab==="wasde"&&(<div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,overflow:"hidden"}}>
  ...
  <div style={{padding:"12px 16px",borderTop:"1px solid "+C.border,display:"flex",gap:8}}><input value={wasdeChatInput} onChange={function(e){setWasdeChatInput(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendWasdeChat();}}} placeholder="Ask about WASDE... (e.g. what will July WASDE show?)" style={{flex:1,background:C.bg,border:"1px solid "+C.border,borderRadius:8,padding:"8px 12px",color:C.text,fontSize:12,outline:"none"}}/><button onClick={sendWasdeChat} disabled={wasdeChatLoading||!wasdeChatInput.trim()} style={{padding:"8px 16px",borderRadius:8,border:"none",background:"linear-gradient(135deg,"+C.blue+","+C.purple+")",color:"#fff",fontSize:12,cursor:"pointer",fontWeight:600,opacity:wasdeChatLoading||!wasdeChatInput.trim()?0.5:1}}>Send</button></div>
</div>)}
